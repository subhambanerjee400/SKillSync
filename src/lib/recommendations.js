...
import { TRADE_INSTITUTES, ROLE_REQUIRED_SKILLS } from '../data/demoData.js';
 ...
 * @returns {{ recommendations: Array, notice: string|null, isFallback: boolean }}
 */
export function generateTradeRecommendations(missingSkills = [], profile = {}) {
  ...
  if (!userTrade) {
    return { recommendations: [], notice: null, isFallback: false };
  }

  const missingNormSet = new Set((missingSkills || []).map(normalize));

  // 1. Trade match (exact match, case-insensitive)
  const tradeInstitutes = (TRADE_INSTITUTES || []).filter(
  ...
  for (const inst of tradeInstitutes) {
    ...
    if (overlappingSkills.length > 0) {
      skillMatchingInstitutes.push({
        ...inst,
        overlappingSkills,
        targetSkill: overlappingSkills[0],
      });
    }
  }
  ...
  if (userLocation) {
    const cityMatches = skillMatchingInstitutes.filter((inst) =>
      userLocation.includes(inst.city.toLowerCase()) ||
      inst.city.toLowerCase().includes(userLocation)
    );
    ...
    if (cityMatches.length > 0) {
      ...
      // Fallback: allow trade+skill matches, but label as "Available in India"
  ...
  if (finalInstitutes.length > 0) {
    const mapped = finalInstitutes.map((inst) => {
      ...
      return {
        id: inst.id,
        title: inst.name,
        subtitle: `${inst.type} Institute • ${inst.city} (${locationLabel})`,
        provider: `${inst.type} Training Center`,
        description: explanation,
        skillsOffered: inst.skillsOffered,
        link: inst.website,
        ctaText: 'Visit Institute Website',
        distanceKm: inst.distanceKm,
        isGovernment: inst.type === 'Government',
      };
    });

    return {
      recommendations: mapped.slice(0, 4),
      notice: null,
      isFallback: false,
    };
  }
  ...
  const fallbackCard = isEmerging
    ? {
        ...
        description: 'No nearby training institute found for your current skill gap. Access accredited national training centers and specialized green-energy skilling schemes.',
    ...
    : {
        ...
        description: 'No nearby training institute found for your current skill gap. Enroll in government-certified vocational training centers across India.',
  ...
  return {
    ...
    notice: 'No nearby training institute found for your current skill gap.',
    isFallback: true,
  };
}
